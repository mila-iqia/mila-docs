    ...
    try:
        ...
        for job_name in subprocess.run(
            ...
            if job_name.strip().startswith(f"{TEST_JOB_NAME}_"):
                subprocess.run(
                    ["scancel", "-u", username, "--name", job_name.strip()],
                    check=True,
                )
            else:
                logger.debug(
...
@pytest.mark.parametrize(
    ("example_dir", "make_reproducible", "sbatch_options"),
    [
        ...
        (EXAMPLES_DIR / "good_practices" / "launch_many_jobs", True, []),
        (EXAMPLES_DIR / "good_practices" / "many_tasks_per_gpu", True, []),
        (EXAMPLES_DIR / "good_practices" / "slurm_job_arrays", True, ["--array", "1-5"]),
        (EXAMPLES_DIR / "good_practices" / "wandb_setup", True, []),
        # Jax examples
        (EXAMPLES_DIR / "frameworks" / "jax_setup", False, []),
        (EXAMPLES_DIR / "frameworks" / "jax", True, []),
    ],
    ids=_test_id,
)
def test_example(
...
def test_checkpointing_example(file_regression: FileRegressionFixture):
    ...
    job: SlurmJob = submit_example(job_script, wait_for_results=False)
    interval_seconds = 1
