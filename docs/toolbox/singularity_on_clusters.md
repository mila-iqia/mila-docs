...
!!! warning "You should always store your container in $SCRATCH!"
    Then reserve a node with srun/sbatch, copy the container and your dataset on
    the node given by SLURM (i.e in `$SLURM_TMPDIR`) and execute the code `<YOUR_CODE>` within the container `<YOUR_CONTAINER>` with:
